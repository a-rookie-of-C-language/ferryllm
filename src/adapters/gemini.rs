...
impl Adapter for GeminiAdapter {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = Result<StreamEvent, AdapterError>> + Send>>, AdapterError>
    {
        ...
        tokio::spawn(async move {
            ...
            // Buffer tool calls until the stream ends, then emit atomically.
            let mut pending_tools: Vec<(u32, GeminiFunctionCall)> = Vec::new();
                    ...
                    for candidate in response.candidates {
                        ...
                        for part in content.parts {
                            ...
                            if let Some(function_call) = part.function_call {
                                ...
                                next_output_index += 1;
                                pending_tools.push((index, function_call));
                            }
                        }
                    }
                }
            ...
            // Close any open text block before emitting tools.
            if text_started {
                if let Some(index) = text_index {
                    let _ = tx.send(Ok(StreamEvent::ContentBlockStop { index }));
                }
            }
            // Emit all buffered tool blocks atomically.
            for (index, fc) in pending_tools {
                let _ = tx.send(Ok(StreamEvent::ContentBlockStart {
                    index,
